...
@onready var arvrorigin = XRHelpers.get_xr_origin(get_node("/root/Main/WalkMode"))
...
func PF_processlocalavatarposition(delta):
	...
	$hand_l.transform = arvrorigin.global_transform.inverse()*LeftHandController.get_node("LeftHand/Hand_Nails_low_L").global_transform
	skelbonescopy($hand_l/Armature/Skeleton3D, LeftHandController.get_node("LeftHand/Hand_Nails_low_L/Armature/Skeleton3D"))
	$hand_r.transform = arvrorigin.global_transform.inverse()*RightHandController.get_node("RightHand/Hand_Nails_low_R").global_transform
	skelbonescopy($hand_r/Armature/Skeleton3D, RightHandController.get_node("RightHand/Hand_Nails_low_R/Armature/Skeleton3D"))
	if projectedhands:
